...
function serializeRoot(node: RootNode): string {
  const attrs =
    node.source.match(/<template(?<attrs>.*?)>/)?.groups?.attrs || "";

  return `<template${attrs}>${node.children
    .map((child) => serializeTemplateItem(child))
    .join("")}</template>`;
}
...
function serializeElement(node: ElementNode): string {
  const props =
    node.props.length > 0
      ? ` ${node.props.map((prop) => serializeTemplateItem(prop)).join(" ")}`
      : "";
...
function serializeAttribute(node: AttributeNode): string {
  const value =
    node.value !== undefined ? `="${serializeTemplateItem(node.value)}"` : "";
...
export function stringify(ast: SFCParseResult) {
  const template = ast.descriptor.template
    ? serializeTemplate(ast.descriptor.template)
    : "";
  const script = ast.descriptor.script
    ? serializeScript(ast.descriptor.script)
    : "";
  const setup = ast.descriptor.scriptSetup
    ? serializeScript(ast.descriptor.scriptSetup)
    : "";
